...
import { DiffCalculator, GetProductCategory, calculatepercent, } from "../Utils/Features.js";
...
export const AdminbashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (MyCache.has("admin-stats"))
        ...
        const LatestTransactionPromise = Order.find({})
            ...
            .limit(4);
        const [ThisMonthUser, ThisMonthProducts, ThisMonthOrder, LastMonthUser, LastMonthProducts, LastMonthOrder, ProductsCount, UserCount, AllOrders, SixMonthAgoOrder, ProductCategories, FemaleUserCount, LatestTransaction,] = await Promise.all([
        ...
        const CategoryCount = await GetProductCategory({
            ProductCategories,
            ProductsCount,
        });
        const UserRatio = {
...
export const AdminbashboardPie = TryCatch(async (req, res, next) => {
    ...
    if (MyCache.has("admin-pie-chart"))
        ...
        const [OrderProcessing, OrderShipped, OrderDelivered, ProductCategories, ProductsCount, productOutStock, Allorders, AllUsers, CustomerUser, AdminUser,] = await Promise.all([
...
export const AdminbashboardLine = TryCatch(async (req, res, next) => {
    ...
    if (MyCache.has("admin-line-chart"))
        ...
        const [TwelveMonthOrder, TwevlveMonthProduct, TwelveMonthUser] = await Promise.all([
            TwelveMonthAgoOrderPromise,
